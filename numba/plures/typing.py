...
def unbox_xnd(typ, obj, c):
    ...
    """
    const_xnd = pycapsule_import(c, "xnd._xnd._API", 2, ir.FunctionType(
        ptr(xnd_t),
        [c.pyapi.pyobj]
    ), name='const_xnd')
...
def box_xnd(typ, val, c):
    ...
    """
    xnd_from_xnd = pycapsule_import(c, "xnd._xnd._API", 5, ir.FunctionType(
        c.pyapi.pyobj,
        [c.pyapi.pyobj, ptr(xnd_t)]
    ), name='xnd_from_xnd')
...
def box_ndt(typ, val, c):
    ...
    """
    ndt_from_type = pycapsule_import(c, "ndtypes._ndtypes._API", 6, ir.FunctionType(
        c.pyapi.pyobj,
        [ptr(ndt_t)]
    ), name='ndt_from_type')
...
def pycapsule_import(c, path, i: int, fntype, name=None):
    ...
    return builder.load(
        builder.bitcast(
            builder.gep(xnd_api, [index(i * 8)], True),
            ptr(ptr(fntype))
        ),
        name=name
    )
